...
class SoccerNetGameState(TrackingDataset):
    def __init__(self,
        ...
        # for split in ["train", "valid", "test", "challenge", "franz", "sn500", "matchmaster"]:
        split = eval_set
        if os.path.exists(self.dataset_path / split):
            vids_filter_set = vids_dict.get(split, [])
            if len(vids_filter_set) == 0 and start_vid is not None and end_vid is not None and split == eval_set:
                start_vid = int(start_vid)
                end_vid = int(end_vid)
                for vid in range(start_vid, end_vid + 1):
                    vids_filter_set.append(f"SNGS-{vid:05d}")
            
            sets[split] = load_set(self.dataset_path / split, nvid, vids_filter_set)
        else:
    ...
    def soccernet_encoding(dataframe: pd.DataFrame, supercategory):
        ...
        if supercategory == "object":
            ...
            # haolin: add bbox_conf column
            dataframe = dataframe[dataframe.columns.intersection(
                ["id", "image_id", "video_id", "track_id", "supercategory",
                 "category_id", "attributes", "bbox_image", "bbox_conf", "bbox_pitch"])]
            dataframe = dataframe.rename(columns={"bbox_conf": "bbox_image_conf"})
...
def dict_to_df_detections(annotation_dict, categories_list):
    ...
    df = df.loc[df['supercategory'] == 'object']        # remove the rows with non-human categories, haolin: but I find that ball is also included in the object category
...
def video_dir_to_dfs(args):
    ...
    split = args['split']
    split_id = ["train", "valid", "test", "challenge", "franz", "sn500", "matchmaster"].index(split) + 1
    # split_id = ["valid",].index(split) + 1
...
def load_set(dataset_path, nvid=-1, vids_filter_set=None):
    ...
    if len(categories_list) == 0:
        ...
        categories_list = [{'id': i + 1, 'name': category, 'supercategory': 'person'} for i, category in # haolin: ball也被算进来了？
