...
class SoccerNetGS(_BaseDataset):
    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        ...
        for annotation in data[key]:
            ...
            if self.eval_space == 'pitch':
                ...
                # haolin: add bbox_image to eval yolo
                bbox_image = annotation["bbox_image"]
                dets[timestep][-1].extend([bbox_image["x"], bbox_image["y"], bbox_image["w"], bbox_image["h"]])
                if is_gt:
                    bbox_image_conf = 1
                else:
                    bbox_image_conf = annotation["bbox_image_conf"]
                dets[timestep][-1].extend([bbox_image_conf])
            ...
            # haolin: add mask to evaluate these attributes
            role_mask = True if self.use_roles else False
            team_mask = True if (self.use_teams and self.use_roles and role in {"player", "goalkeeper"}) or (self.use_teams and not self.use_roles) else False
            jersey_mask = True if (self.use_jersey_numbers and self.use_roles and role in {"player"}) or (self.use_jersey_numbers and not self.use_roles) else False
    ...
    def get_preprocessed_seq_data(self, raw_data, cls):
        ...
        data_keys = ['gt_ids', 'tracker_ids', 'gt_dets', 'tracker_dets', 'similarity_scores', 'gt_attributes', 'tracker_attributes']
        ...
        for t in range(raw_data['num_timesteps']):
            ...
            gt_roles = np.array([gt_extra['role'] for gt_extra in raw_data['gt_extras'][t]])[gt_class_mask]
            gt_teams = np.array([gt_extra['team'] for gt_extra in raw_data['gt_extras'][t]])[gt_class_mask]
            gt_jerseys = np.array([gt_extra['jersey'] for gt_extra in raw_data['gt_extras'][t]])[gt_class_mask]
            gt_role_masks = np.array([gt_extra['role_mask'] for gt_extra in raw_data['gt_extras'][t]])[gt_class_mask]
            gt_team_masks = np.array([gt_extra['team_mask'] for gt_extra in raw_data['gt_extras'][t]])[gt_class_mask]
            gt_jersey_masks = np.array([gt_extra['jersey_mask'] for gt_extra in raw_data['gt_extras'][t]])[gt_class_mask]
            gt_extras = {'role': gt_roles, 'team': gt_teams, 'jersey': gt_jerseys, 'role_mask': gt_role_masks, 'team_mask': gt_team_masks, 'jersey_mask': gt_jersey_masks}

            tracker_class_mask = np.atleast_1d(raw_data['tracker_classes'][t] == cls_id)
            tracker_class_mask = tracker_class_mask.astype(bool)
            tracker_ids = raw_data['tracker_ids'][t][tracker_class_mask]
            tracker_dets = raw_data['tracker_dets'][t][tracker_class_mask]
            tracker_roles = np.array([tracker_extra['role'] for tracker_extra in raw_data['tracker_extras'][t]])[tracker_class_mask]
            tracker_teams = np.array([tracker_extra['team'] for tracker_extra in raw_data['tracker_extras'][t]])[tracker_class_mask]
            tracker_jerseys = np.array([tracker_extra['jersey'] for tracker_extra in raw_data['tracker_extras'][t]])[tracker_class_mask]
            tracker_role_masks = np.array([tracker_extra['role_mask'] for tracker_extra in raw_data['tracker_extras'][t]])[tracker_class_mask]
            tracker_team_masks = np.array([tracker_extra['team_mask'] for tracker_extra in raw_data['tracker_extras'][t]])[tracker_class_mask]
            tracker_jersey_masks = np.array([tracker_extra['jersey_mask'] for tracker_extra in raw_data['tracker_extras'][t]])[tracker_class_mask]
            tracker_extras = {'role': tracker_roles, 'team': tracker_teams, 'jersey': tracker_jerseys, 'role_mask': tracker_role_masks, 'team_mask': tracker_team_masks, 'jersey_mask': tracker_jersey_masks}
            
            similarity_scores = raw_data['similarity_scores'][t][gt_class_mask, :][:, tracker_class_mask]

            data['tracker_ids'][t] = tracker_ids
            data['tracker_dets'][t] = tracker_dets
            data['tracker_attributes'][t] = tracker_extras
            data['gt_ids'][t] = gt_ids
            data['gt_dets'][t] = gt_dets  # FIXME assert not 0 size
            data['gt_attributes'][t] = gt_extras
